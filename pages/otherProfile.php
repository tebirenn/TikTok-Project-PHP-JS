...
if (isset($_GET["user"])) {
    $_SESSION["otherUser"] = $_GET["user"];
}
$otherUser = $_SESSION["otherUser"];
$query_to_profile = mysqli_query($conn, "SELECT * FROM profile WHERE username=\"$otherUser\"");
$res_to_profile = mysqli_fetch_assoc($query_to_profile);
$otherName = $res_to_profile["name"];
$username = $_SESSION["username"];
$avatarName = $res_to_profile["avatar_name"];

if (isset($_POST["sub"])) {
    $query_to_sub = mysqli_query($conn, "SELECT * FROM subscribed WHERE sub_from=\"$username\" AND sub_to=\"$otherUser\";");

    if (mysqli_num_rows($query_to_sub) == 0) {
    ...
    header("Location: $BASE_URL/pages/otherProfile.php");
}
...
<head>
    ...
    <link rel="stylesheet" href="../styles/pages/otherProfile.css">
    ...
    <section>
        ...
        <div id="section-right">
            <div id="profile-main-info">
                    ...
                    <span>
                        ...
                        ?>
                            <form action="otherProfile.php" method="POST">
                                <span>
                                    <input type="text" style="display: none;" name="sub">
                                    <button type="submit" id="sub-button">Подписаться</button>
                                </span>
                            </form>
                        <?php
                        }
                ...
                <div>
                    <p><b>
                        <?php
                        $otherUser = $_SESSION["otherUser"];
                        echo mysqli_num_rows(mysqli_query($conn, "SELECT * FROM subscribed WHERE sub_from=\"$otherUser\";"));
                        ?>
                    </b> Подписки</p>
                    <p><b>
                        <?php
                        echo mysqli_num_rows(mysqli_query($conn, "SELECT * FROM subscribed WHERE sub_to=\"$otherUser\";"));
                        ?>
                    </b> Подписчики</p>
                    <p><b>187.7M</b> Лайки</p>
                </div>
                <div>
            ...
            <div id="profile-videos">
                ...
                <?php
                    $otherUser = $_SESSION["otherUser"];
                    $query_to_all_videos = mysqli_query($conn, "SELECT * FROM videos_info WHERE from_who=\"$otherUser\";");
